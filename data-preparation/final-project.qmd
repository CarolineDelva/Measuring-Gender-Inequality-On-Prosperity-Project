...
```{r}
write.csv(gender_inequality_and_economic_indicators_dataset, "data/processed-data/gender_inequality_and_economic_indicators_dataset_merged.csv", row.names = FALSE)
...
# Grouping by income group and imputing medians for each group

# Foreign Direct Investment
Foreign_direct_investment_median <- gender_inequality_and_economic_indicators_dataset %>%
    group_by(Income.Group) %>%
    summarize(median_fdi = median(Foreign_direct_investment, na.rm = TRUE))

gender_inequality_and_economic_indicators_dataset <- gender_inequality_and_economic_indicators_dataset %>%
    left_join(Foreign_direct_investment_median, by = "Income.Group") %>%
    mutate(Foreign_direct_investment = ifelse(is.na(Foreign_direct_investment), median_fdi, Foreign_direct_investment)) %>%
    select(-median_fdi)

# Agriculture, Forestry, and Value Added
agriculture_forestry_value_added_median <- gender_inequality_and_economic_indicators_dataset %>%
    group_by(Income.Group) %>%
    summarize(median_agri = median(agriculture_forestry_value_added, na.rm = TRUE))

gender_inequality_and_economic_indicators_dataset <- gender_inequality_and_economic_indicators_dataset %>%
    left_join(agriculture_forestry_value_added_median, by = "Income.Group") %>%
    mutate(agriculture_forestry_value_added = ifelse(is.na(agriculture_forestry_value_added), median_agri, agriculture_forestry_value_added)) %>%
    select(-median_agri)

# GNI per Capita
GNI_per_capita_median <- gender_inequality_and_economic_indicators_dataset %>%
    group_by(Income.Group) %>%
    summarize(median_gni = median(GNI.per.capita, na.rm = TRUE))

gender_inequality_and_economic_indicators_dataset <- gender_inequality_and_economic_indicators_dataset %>%
    left_join(GNI_per_capita_median, by = "Income.Group") %>%
    mutate(GNI.per.capita = ifelse(is.na(GNI.per.capita), median_gni, GNI.per.capita)) %>%
    select(-median_gni)

# Net Barter Terms of Trade Index
net_barter_terms_of_trade_index_median <- gender_inequality_and_economic_indicators_dataset %>%
    group_by(Income.Group) %>%
    summarize(median_trade = median(net_barter_terms_of_trade_index, na.rm = TRUE))

gender_inequality_and_economic_indicators_dataset <- gender_inequality_and_economic_indicators_dataset %>%
    left_join(net_barter_terms_of_trade_index_median, by = "Income.Group") %>%
    mutate(net_barter_terms_of_trade_index = ifelse(is.na(net_barter_terms_of_trade_index), median_trade, net_barter_terms_of_trade_index)) %>%
    select(-median_trade)

# GDP per Person Employed
gdp_per_person_employed_median <- gender_inequality_and_economic_indicators_dataset %>%
    group_by(Income.Group) %>%
    summarize(median_gdp = median(gdp_per_person_employed, na.rm = TRUE))

gender_inequality_and_economic_indicators_dataset <- gender_inequality_and_economic_indicators_dataset %>%
    left_join(gdp_per_person_employed_median, by = "Income.Group") %>%
    mutate(gdp_per_person_employed = ifelse(is.na(gdp_per_person_employed), median_gdp, gdp_per_person_employed)) %>%
    select(-median_gdp)
...
```
```{r}
write.csv(gender_inequality_and_economic_indicators_dataset, "data/processed-data/gender_inequality_and_economic_indicators_dataset_clean_final.csv", row.names = FALSE)
```
