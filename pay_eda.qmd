...
df <- df %>% filter(Income.Group != "Not classified")
```
...
# Plot the mean WBL score by Income Group with individual points
ggplot(df %>% filter(!is.na(Income.Group)), aes(x = factor(Income.Group, levels = income_levels), y = WBL.INDEX, color = Income.Group)) +
    geom_point(position = position_jitter(width = 0.2), alpha = 0.6) +
    geom_line(data = income_group_mean_wbl, aes(x = factor(Income.Group, levels = income_levels), y = mean_wbl, group = 1), color = "black", size = 1) +
    geom_point(data = income_group_mean_wbl, aes(x = factor(Income.Group, levels = income_levels), y = mean_wbl), color = "black", size = 3) +
    theme_minimal() +
    labs(title = "WBL Scores by Income Group", 
         x = "Income Group", 
         y = "WBL Score") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(hjust = 0.5, face = "bold"))

ggsave("income_group_wbl_scores.png")
```
...
```{r}
#trying different style of graph
df <- df %>% filter(Income.Group != "Not classified")
income_levels <- c("Low income", "Lower middle income", "Upper middle income", "High income")
...
# Plot the mean WBL score by Income Group with individual points and vertical mean lines
ggplot(df, aes(x = WBL.INDEX, color = Income.Group)) +
    # Add individual data points
    geom_point(aes(y = factor(Income.Group, levels = income_levels)), position = position_jitter(width = 0.2, height = 0), alpha = 0.6) +
    # Add vertical mean lines with legend
    geom_vline(data = income_group_mean_wbl, aes(xintercept = mean_wbl, color = Income.Group), linetype = "solid", size = 1, show.legend = TRUE) +
    # Customize colors
    scale_color_manual(
        values = c("Low income" = "blue", 
    ...
    theme_minimal() +
    labs(
        title = "WBL Scores by Income Group", 
        x = "WBL Score", 
        y = "Income Group"
    ) +
    theme(
        axis.text = element_text(size = 14),          # Increase size of axis tick labels
        axis.title = element_text(size = 16, face = "bold"),  # Increase size of axis titles
        plot.title = element_text(size = 20, hjust = 0.5, face = "bold"), # Increase size of title
        legend.text = element_text(size = 14),       # Increase size of legend text
        legend.title = element_text(size = 16, face = "bold"), # Increase size of legend title
        legend.position = "top"                      # Place legend at the top
    )

ggsave("anova_income_group_wbl_scores.png", width = 12, height = 10)
```

```{r}
# df without missing income group
anova_result_income_wbl_all <- aov(WBL.INDEX ~ Income.Group, data = df)
summary(anova_result_income_wbl_all)
```

```{r}

# Perform ANOVA test for WBL index score and Income Group without the High income group
anova_result_income_wbl_wo_high <- aov(WBL.INDEX ~ Income.Group, data = df %>% filter(Income.Group != "High income"))
summary(anova_result_income_wbl_wo_high)
```

```{r}
tukey_results <- TukeyHSD(anova_result_income_wbl_all)
print(tukey_results)
```

```{r}
tukey_results <- TukeyHSD(anova_result_income_wbl_wo_high)
print(tukey_results)
...
### permutation tests
perm_diff <- function() {
    permuted_with <- sample(df$WBL_Group)
    mean(df$gdp_per_person_employed[permuted_with == "High"], na.rm = TRUE) -
    mean(df$gdp_per_person_employed[permuted_with == "Low"], na.rm = TRUE) 
}

df$WBL_Group <- ifelse(df$WBL.INDEX >= median(df$WBL.INDEX, na.rm = TRUE), "High", "Low")

df$WBL_Group <- factor(df$WBL_Group)

unique(df$WBL_Group)
table(df$WBL_Group)

# observed difference in gdp means with high income
observed_diff_with <- mean(df$gdp_per_person_employed[df$WBL_Group == "High"], na.rm = TRUE) -
    mean(df$gdp_per_person_employed[df$WBL_Group == "Low"], na.rm = TRUE)

# permutation test
perm_diff_with <- replicate(5000, perm_diff())

# # calculate p value
# p_value_with <- mean(abs(perm_diff_with) >= abs(observed_diff_with))

# plot histogram of permutation test results
ggplot(data.frame(perm_diff_with), aes(x = perm_diff_with)) +
    geom_histogram(binwidth = 500, fill = "lightblue", color = "black") +
    geom_vline(xintercept = observed_diff_with, color = "red", linetype = "dashed", size = 1.2) +
    annotate("text", x = observed_diff_with, y = 20, label = "Observed\nDifference", color = "red", angle = 90, vjust = -0.5, size = 5) +
    labs(title = "Permutation Test: GDP Difference by WBL Group",
         x = "Difference in Mean GDP (High - Low WBL Group)",
         y = "Frequency") +
    theme_minimal() +
...
gdp_with <- simulation_gdp(df, 10000)
